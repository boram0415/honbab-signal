...
// 색 원 마커 이미지 (저장한 곳은 빨간 테두리)
function markerSvg(color: string, saved: boolean): string {
  const ring = saved
    ? `<circle cx="12" cy="12" r="10.5" fill="none" stroke="#f43f5e" stroke-width="2.5"/>`
    : "";
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><circle cx="12" cy="12" r="7" fill="${color}" stroke="#fff" stroke-width="2.5"/>${ring}</svg>`;
